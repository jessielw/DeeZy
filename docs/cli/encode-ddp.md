...
usage: DeeZy encode ddp [-h] [--ffmpeg FFMPEG] [--truehdd TRUEHDD] [--dee DEE]
                        ...
                        [--no-surround-90-deg-phase-shift] [--upmix-50-to-51]
...
```bash
deezy encode ddp --max-parallel 3 --batch-summary-output --batch-output-dir "C:\\out\\ddp" "C:\\media\\album\\"
```

- Force stereo output and preview filename template:

```bash
deezy encode ddp --channels 2 --output-template "{title}_{channels}.ddp" --output-preview "C:\\media\\song.wav"
```
