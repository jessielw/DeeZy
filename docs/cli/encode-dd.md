...
```bash
deezy encode dd --delay=150ms "C:/media/album/*.mkv"
```

- Reuse extracted temporary files (fast when rerunning):

```bash
deezy encode dd --reuse-temp-files "C:/media/album/*.mkv"
```

- Limit FFmpeg concurrency separate from DEE:

```bash
deezy encode dd --max-parallel 4 --limit-ffmpeg 2 --batch-summary-output "C:/media/album/*.mkv"
```
